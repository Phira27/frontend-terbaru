...
function Pm10Chart() {
  ...
  const fetchData = async () => {
    try {
      ...
      const formattedData = pasirianData.map((item, index) => ({
        ...
        pasirian_PM10: item.PM10_concentration,
        lumajang_PM10: lumajangData[index] ? lumajangData[index].PM10_concentration : null,
        senduro_PM10: senduroData[index] ? senduroData[index].PM10_concentration : null
      })).reverse();
  ...
  return (
    <div className="mb-8">
      <h2 className="text-3xl font-bold text-gray-800 mb-4 text-center">PM 10 Concentration</h2>
      {isLoading ? (
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line type="monotone" dataKey="pasirian_PM10" stroke="#8884d8" strokeWidth={2} dot={false} name="Pasirian" />
            <Line type="monotone" dataKey="lumajang_PM10" stroke="#82ca9d" strokeWidth={2} dot={false} name="Lumajang" />
            <Line type="monotone" dataKey="senduro_PM10" stroke="#ffc658" strokeWidth={2} dot={false} name="Senduro" />
          </LineChart>
        </ResponsiveContainer>
      ) : (
